                ...
                implements SearchTreeInterface<T>
{
    ...
    {
        setRootNode(addEntry(getRootNode(), newEntry));
        return true;
    } // end add
    
   private BinaryNode<T> addEntry(BinaryNode<T> node, T newEntry)
    {
        BinaryNode<T> result = node;
        
        if (node == null)
        {
            result = new BinaryNode<>(newEntry);
        }
        else
        {
            T nodeData = node.getData();
            int comparison = newEntry.compareTo(nodeData);
            
            if (comparison < 0)
            {
                node.setLeftChild(addEntry(node.getLeftChild(), newEntry));
            }
            else if (comparison > 0)
            {
                node.setRightChild(addEntry(node.getRightChild(), newEntry));
            }
            else
            {
